...
- `src/main/tray-icon-path.ts` — `resolveTrayIconPath` 纯函数模块(不依赖 Electron 运行时,便于单测)。
...
- `src/main/index.ts` — `getTrayIconPath()` 改为从 `./tray-icon-path` 导入 `resolveTrayIconPath` 并指向 `trayIconTemplate.png`。
- `package.json` — `build` 字段新增 `extraResources`,把 PNG 复制到 `Contents/Resources/`。

> 设计调整说明:`resolveTrayIconPath` 原计划直接 export 自 `index.ts`,但 `index.ts` 顶层调用了 `app.requestSingleInstanceLock()` 等 Electron 运行时 API,vitest 加载时会因 `app` 未定义而崩溃,无法直接 `import { resolveTrayIconPath } from './index'`。故把纯函数拆到独立 `tray-icon-path.ts`,既符合设计 §2 "纯函数,不依赖 Electron 运行时"的原意,改动也比 mock electron / 重构 index.ts 顶层都小。
...
Expected:FAIL,失败信息提示 `./tray-icon-path` 模块或 `resolveTrayIconPath` 不存在(模块解析错)。

确认是预期的失败再进入 Task 6。

---

### Task 6:新建 tray-icon-path.ts 纯函数模块并接入 index.ts

**Files:**
- Create: `src/main/tray-icon-path.ts`
- Modify: `src/main/index.ts`

- [ ] **Step 1: 创建 tray-icon-path.ts 纯函数模块**

写入 `src/main/tray-icon-path.ts`:
...
模块仅依赖 `node:fs`,不依赖 Electron 运行时,可在 Vitest 下直接 import。
...
在文件顶部 import 区域(其他 `./xxx` import 相邻处)新增:

```ts
import { resolveTrayIconPath } from './tray-icon-path'
```

定位现有 `getTrayIconPath` 函数(约第 544-554 行),整段替换为:
...
注意:`join` 在文件顶部已 import;`existsSync` 此处已不再使用,如果当前 `index.ts` 顶部 import 行只为 `getTrayIconPath` 旧实现引入了 `existsSync`,确认是否仍被其它代码用到——若不再使用,从 `node:fs` 的 import 列表里移除它,保持顶部 import 干净。

- [ ] **Step 4: 跑 typecheck 与全套测试**
...
```bash
git add src/main/tray-icon-path.ts src/main/tray-icon-path.test.ts src/main/index.ts
git commit -m "refactor: 抽 resolveTrayIconPath 到独立模块, tray 图标指向 template PNG"
```
...
- 设计 §2 index.ts `resolveTrayIconPath` 抽函数 + `getTrayIconPath` 改路径 → Task 5 + Task 6(实施时拆到独立模块 `tray-icon-path.ts`,见 File Structure 调整说明)✓
